    ...
    (function($) {
        $.fn.JSNESUI = function(roms) {
            ...
            var UI = function(nes) {
                ...
                /*
                 * Create UI
                 * 屏幕(canvas)保留在显示器内，选择游戏/重启/暂停/声音等控制
                 * 全部放到显示器外部的 #ext-controls 容器中
                 */
                ...
                var extControls = $('#ext-controls');
                self.romContainer = $('<div class="nes-roms"></div>').appendTo(extControls);
                self.romSelect = $('<select></select>').appendTo(self.romContainer);
                self.controls = $('<div class="nes-controls"></div>').appendTo(extControls);
                self.buttons = {
                ...
                self.status = $('<p class="nes-status">Booting up...</p>').appendTo(extControls);
                ...
                /*
                 * 画面默认在显示器内放大显示（由 CSS 控制），不再需要放大按钮
                 */
                ...
                /*
                 * 虚拟按键统一绑定：优先 Pointer Events（同时覆盖 PC 鼠标、PC 触屏、移动端触摸），
                 * 不支持时降级为 touch + mouse 双绑定。
                 * 旧实现只绑了 touchstart/touchend，导致 PC 上鼠标点击 Select/Pause/A/B 无效。
                 */
                function bindVirtualButton(id, keyCode, withActive) {
                    var el = document.getElementById(id);
                    if (!el) return;
                    var pressed = false;

                    // 防止触屏上按钮触发滚动/缩放，保证 pointer 事件完整
                    ...
                    function press(e) {
                        ...
                        if (withActive) $(el).addClass('active');
                    }
                    ...
                    function release(e) {
                        ...
                        if (withActive) $(el).removeClass('active');
                    }

                    if (window.PointerEvent) {
                        el.addEventListener('pointerdown', function(e) {
                            // 捕获指针，手指/鼠标滑出按钮外释放时也能收到 pointerup
                    ...
                    // 兜底：任何情况下窗口失焦都释放按键，避免"卡键"
                    window.addEventListener('blur', release);
                }

                bindVirtualButton('joystick_btn_up', 87);
                bindVirtualButton('joystick_btn_down', 83);
                bindVirtualButton('joystick_btn_left', 65);
                bindVirtualButton('joystick_btn_right', 68);
                bindVirtualButton('joystick_btn_A', 74);
                bindVirtualButton('joystick_btn_B', 75);
                bindVirtualButton('joystick_btn_select', 17, true);
                bindVirtualButton('joystick_btn_start', 13, true);
            ...
            UI.prototype = {
                loadROM: function() {
                    ...
                    $.ajax({
                        // encodeURI 与 sw.js 预缓存时的编码方式保持一致，保证离线命中缓存
